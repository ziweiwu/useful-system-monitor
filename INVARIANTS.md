...
| I-10b | The heap stays flat across a long run: **0.6 KB/render measured, against 173.7 KB/render** before the fix. `dist/cli.js` defaults `NODE_ENV` to `production` **before React is imported**, because React's development reconciler emits a `performance.measure()` per component per commit and Node never evicts user-timing entries — which reached V8's 4 GB ceiling and died on `Ineffective mark-compacts near heap limit`. Nothing statically imported by the entry point may reach React | `cli.tsx` · `core/reactEnv.ts` · `test/entry.test.ts` · `npm run verify:longrun` |
